...
use crate::{
    ...
    types::local_cell_id::LocalCellId, utils::flags::Flags,
};
...
pub struct CEnvCell {
    ...
    pub shared_cells: Vec<LocalCellId>,
}
...
impl CEnvCell {
    pub fn new(data: &mut BinaryReader) -> io::Result<Self> {
        ...
        let stab_list = data.read_list(|b| b.read_local_cellid(), 4)?;
        data.align(4);
        let shared_cells = data.read_list(|b| b.read_local_cellid(), 4)?;
        data.align(4);
